...
class Snake
{
	...
	public:
        ...
        void mapChange(int next_map[30][60]){ // 맵 바꿔주기
            ...
            checkWall(next_map); //벽의 위치 저장하는 벡터 생성
         ...
         void checkForwall() // 벽에 닿으면 죽음
		{
            for(int i=0; i<countW; i++){
                if(body[0]==wall1[i] && body[0] != gate[0] && body[0]!= gate[1]){ // 첫뺀째 마디가 벽에 닿았고, 그게 gate가 아니라면 죽음!
					exit = true;
				}
        ...
        void checkGate() // 몸이 다 통과할때까지 머리 방향 설정 ->한쪽 gate로 나가면 다른쪽 gate로 들어오도록!
        {
            if(body[0] == gate[0]){
                body[0] = gate[1];
                countG++;
